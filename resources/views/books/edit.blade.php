...
<div class="max-w-2xl mx-auto bg-white rounded-lg shadow overflow-hidden">
    ...
    <form action="{{ route('books.update', $book->id) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3" 
                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $book->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
